...
void modelGen::setImage(std::string& strFileName) {
	...
	cv::imwrite("res0.png", m_edgeImage);
	{
		...
		for (int j = 0; j < tempImage.rows; ++j)
		{
			...
			{
				uchar val = *pPixel++;
				//이부분을 에지의 에지일때만 고려하도록 바꾸면 성능 향상됨
				if (val < 244)
			...
			for (int jj = -1; (bHeadNotFound || bHEyebrowsNotFound || bNoseNotFound) && (jj <= 1); ++jj)
			{
				for (int ii = -1; (bHeadNotFound || bHEyebrowsNotFound || bNoseNotFound) && (ii <= 1); ++ii)
				{
					if ((ii || jj) && ((i + ii) >= 0) && ((i + ii) < m_nWidth) && ((j + jj) >= 0) && ((j + jj) < m_nHeight))
					{
						...
						{
							m_pFacialComponentBoundaryMap[index] |= unsigned char(0b10000000);	//head영역의 바운더리는 MSB를 1로 설정
						...
						{
							m_pFacialComponentBoundaryMap[index] |= unsigned char(0b01000000);	//눈썹, 입술, 헤어 영역의 바운더리는 두번째 비트를 1로 설정
						...
						{
							m_pFacialComponentBoundaryMap[index] |= unsigned char(0b00100000);	//코 영역의 바운더리는 세번째 비트를 1로 설정
...
void modelGen::generate3Dmodel(std::string& strFileName, float fProductRadius)
{
	...
	//Delaunay Triangulation 수행(rear)
	tessellateRear();
...
void modelGen::exportModel(std::string& strFileName, float fProductRadius)
{
	...
	*(unsigned int*)(pMem + indexMem) = (nFaceNum + nFaceNumRear + nNumSideFaces);
	...
	float fDegree = 2.5f;						//5도 간격 (degree)
	float fAngle = 3.141592f / 180 * fDegree;	//5도 간격 (radian)
	...
	FILE* pFile;
	errno_t err = fopen_s(&pFile, strFileName.c_str(), "wb");
	if (err != 0)
	{
...
void modelGen::computeLOD()
{
	...
	for (int level = 2; level < 512; level *= 2)
	{
		...
		for (int jbegin = 0; jbegin < (m_nHeight - 1); jbegin += level)
		{
			...
			{
				std::vector<int> xpos;	//9개의 버텍스 x좌표
				std::vector<int> ypos;	//9개의 버텍스 y좌표
				...
				//만약 4개의 모퉁이 버텍스가 모두 radius바깥이라면 
				...
				{
					if (isFourPartsFlat(xpos, ypos))	//4개의 서브파트가 이전 레벨에서 faltten 되었다면 ture, 따라서 추가로 flatten 수행
					{
						bool bAllRemovable = true;

						//상단과 좌측 에지에 대한 검사는 동일 레벨에서 중복되므로 이미 검사했다면 pass
