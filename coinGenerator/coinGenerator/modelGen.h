...
class modelGen
{
	...
	int	  m_nModelSize;					//이거 나중에 설정해야 함: 모델의 진짜 크기
	float m_fTextHeight = 8.f;			//이거 나중에 설정해야 함: 텍스트의 높이
	const float m_fLodError = 0.25f;	//이거 나중에 설정해야 함: LOD 과정에서 높이오차 허용 범위
	const float m_fLodErrorRear = 0.125f;	//이거 나중에 설정해야 함: LOD 과정에서 높이오차 허용 범위
	//const float m_fLodError = 0.f;	//이거 나중에 설정해야 함: LOD 과정에서 높이오차 허용 범위
	...
	float* m_pDistanceMapEyebrow = NULL;		//눈썹, 입술, 헤어 바운더리에 의한 디스턴스맵
	float* m_pDistanceMapNose = NULL;			//코 영역 바운더리에 의한 디스턴스맵
	float* m_pDistanceMapNoseDorsum = NULL;		//콧등에 의한 디스턴스맵
